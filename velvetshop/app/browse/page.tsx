    ...
    <div className="min-h-screen bg-[#0a0a0a] text-[#e8e0d0] font-serif">
      ...
      <nav className="fixed top-0 w-full bg-black/95 border-b border-[#2a2a2a] z-50">
        <div className="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-8 text-sm">
            ...
            <Link href="/browse" className="text-[#c8ff00]">
              Browse
            </Link>
          </div>
      ...
      <div className="pt-24 px-6">
        ...
        <div className="max-w-7xl mx-auto mb-12">
          <h1 className="text-5xl font-bold mb-8">Browse Reptiles</h1>
        ...
        <div className="max-w-7xl mx-auto pb-24">
          ...
          {filteredListings.length === 0 && (
            <div className="text-center py-20">
              <p className="text-gray-400 text-xl">No reptiles found matching your search.</p>
            </div>
          )}
        </div>
      </div>
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="text-right">
                      <div className="text-black font-bold text-2xl">
                        {getCurrency(selectedListing.country)}
                  ...
                  <div className="flex gap-2 mb-4">
                    <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm">
                      {selectedListing.gender || "Unknown"}
                    </span>
                    <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm">
                      {selectedListing.health || "Healthy"}
                    </span>
                    <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm">
                      {selectedListing.availability || "Available"}
                    </span>
                  </div>
                  ...
                  >
                    Add to Cart - {getCurrency(selectedListing.country)}
                    {selectedListing.price}
                  </button>
                </div>
              </div>
      ...
      {showCart && (
        <div className="fixed inset-y-0 right-0 w-full md:w-96 bg-[#111] border-l border-[#2a2a2a] z-[100] p-6 overflow-auto mt-16">
          <div className="flex justify-between mb-8">
            ...
            <button onClick={() => setShowCart(false)} className="text-3xl">
              ✕
            </button>
          </div>
              ...
              {cart.map(item => (
                <div key={item.id} className="flex gap-4 mb-6 border-b border-[#2a2a2a] pb-6">
                  <div className="w-20 h-20 bg-black rounded-xl overflow-hidden flex-shrink-0">
                    {item.image_url ? (
                      <img src={item.image_url} className="object-cover w-full h-full" />
                    ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[#c8ff00]">
                      {getCurrency(item.country)}{item.price}
                    </p>
                  </div>
                  <button
